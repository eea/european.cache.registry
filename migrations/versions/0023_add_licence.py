...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "date_created", sa.Date(), server_default=sa.text("now()"), nullable=True
        ),
        sa.Column("date_updated", sa.Date(), nullable=True),
        sa.Column("undertaking_id", sa.Integer(), nullable=True),
        sa.ForeignKeyConstraint(
            ["undertaking_id"], ["undertaking.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["delivery_id"], ["delivery_licence.id"], ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("id"),
    )
    ...
    op.create_table(
        ...
        sa.Column(
            "organization_country_name_orig", sa.String(length=100), nullable=True
        ),
        sa.Column("custom_procedure_name", sa.String(length=100), nullable=True),
        sa.Column(
            "international_party_country_name", sa.String(length=100), nullable=True
        ),
        sa.Column(
            "international_party_country_name_orig",
