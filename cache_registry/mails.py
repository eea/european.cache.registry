...
from cache_registry.models import MailAddress, OldCompany, OldCompanyLink, Undertaking
...
def send_mail(subject, html, recipients):
    ...
    try:
        ...
        print("Wrong username/password. " "Please review their values in settings.py")
        return False
...
def send_match_mail(match, **kwargs):
    ...
    if not host:
        message = (
            "No BDR_ENDPOINT_URL was set in order to send notification emails. "
...
def send_wrong_match_mail(user, company_id, domain):
    ...
    if not hd:
        message = (
            "No BDR_HELP_DESK_MAIL was set in order to send the wrong "
    ...
    company = Undertaking.query.filter_by(external_id=company_id, domain=domain).first()
    link = OldCompanyLink.query.filter_by(undertaking=company).first()
    kwargs = {
        "user": user,
...
def send_unmatch_mail(user, company_id, oldcompany_id, oldcollection_path, domain):
    template = "mails/unmatch.html"
    subject = "BDR - Unmatch alert"
    company = Undertaking.query.filter_by(external_id=company_id, domain=domain).first()
    oldcompany = OldCompany.query.filter_by(external_id=oldcompany_id).first()
    kwargs = {
        "user": user,
        "company_name": company.name,
