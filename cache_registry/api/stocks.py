...
from cache_registry.api.views import ApiView, ListView
from cache_registry.models import Stock, Undertaking, db
...
class StocksUndertakingListView(ApiView):
    ...
    def post(self, **kwargs):
        ...
        years = [
            stock.year
            for stock in Stock.query.filter_by(undertaking=undertaking)
            .distinct(Stock.year)
            .all()
        ]
        context = {}
        context[external_id] = {}
        for year in years:
            context[external_id][year] = [
                self.serialize(u) for u in undertaking.stocks.filter_by(year=year).all()
            ]
        return context
...
class StocksYearListView(ListView):
    ...
    def post(self, **kwargs):
        ...
        context = {"year": [self.serialize(u) for u in stocks]}
        return context
