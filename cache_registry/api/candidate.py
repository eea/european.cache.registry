...
class CandidateList(ApiView):
    def get(self, **kwargs):
        ...
        for company, links in candidates:
            links_data = [{"name": link.oldcompany.name} for link in links]
            company_data = {
            ...
            data.append({"undertaking": company_data, "links": links_data})
        return data
    ...
    def get(self, undertaking_id, oldcompany_id):
        ...
        if domain:
            undertaking = Undertaking.query.filter_by(
                domain=domain, external_id=undertaking_id
            ).first()
        else:
            undertaking = Undertaking.query.filter_by(
...
class CandidateUnverify(ApiView):
    def post(self, domain, undertaking_id):
        user = request.form["user"]
        link = unverify_link(
            undertaking_id=undertaking_id, user=user, domain=domain
        ) or abort(404)
