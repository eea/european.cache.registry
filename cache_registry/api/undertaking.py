...
from cache_registry.models import Undertaking, EuLegalRepresentativeCompany, db
...
class UndertakingListView(ListView):
    ...
    def serialize(cls, obj, **kwargs):
        ...
        _strip_fields = ("address_id", "represent_id", "oldcompany_id")
        ...
        data.update(
            {
                ...
                "country_history": [
                    country_hist.code for country_hist in obj.country_history
                ],
                ...
                "businessprofile": ",".join(
                    [
                        businessprofiles.highleveluses
...
class UndertakingFilterCountView(ListView):
    ...
    def get(self, **kwargs):
        ...
        if "name" in request.args:
            ...
            qs = [
                u for u in qs if str_matches(u.represent.name, request.args["OR_name"])
            ]
            count = len(qs)
        else:
            count = qs.count()
...
class UndertakingDetailView(DetailView):
    ...
    def get_object(self, pk, **kwargs):
        ...
        return self.model.query.filter_by(domain=domain, external_id=pk).first_or_404()
    ...
    def serialize(cls, obj, **kwargs):
        ...
        for representative in obj.represent_history:
            if representative.address.country.code == obj.country_code:
                continue
            if representative.address.country.code in [
                r.address.country.code for r in history
            ]:
                continue
            if representative.address.country.code in [
                country.code for country in obj.country_history
            ]:
                continue
