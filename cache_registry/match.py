...
def log_match(
    company_id, oldcompany_id, verified, user, domain, oldcompany_account=None
):
...
def add_link(company_id, oldcompany_id):
    link = models.OldCompanyLink.query.filter_by(
        undertaking_id=company_id, oldcompany_id=oldcompany_id
...
def get_unverified_companies(domains):
    return models.Undertaking.query.filter(
        or_(
            models.Undertaking.oldcompany_verified == None,
...
def get_unverified_non_eu_no_represent_companies():
    return [
        company
        for company in get_unverified_companies([FGAS])
        .filter_by(represent_id=None)
        .all()
...
def get_oldcompanies_for_matching():
    ...
    obligations = current_app.config.get("MANUAL_VERIFY_ALL_COMPANIES", [ODS])
...
def get_candidates(external_id, domain):
    company = models.Undertaking.query.filter_by(
...
def verify_link(undertaking_id, oldcompany_id, user):
    ...
    link = models.OldCompanyLink.query.filter_by(
        undertaking=undertaking, oldcompany=oldcompany
    ).first()
    if not link:
        return None
    link.verified = True
...
def unverify_link(undertaking_id, domain, user):
    u = models.Undertaking.query.filter_by(
        external_id=undertaking_id, domain=domain
    ).first()
    if not u or not u.oldcompany_verified:
        return None
    link = models.OldCompanyLink.query.filter_by(
        undertaking_id=undertaking_id, oldcompany_id=u.oldcompany_id
    ).first()
    if link:
        link.verified = False
...
def verify_manual(undertaking_id, domain, oldcompany_account, user):
    u = models.Undertaking.query.filter_by(
        external_id=undertaking_id, domain=domain.upper()
    ).first()
    if not u:
        return None
    u.oldcompany = None
...
def call_run():
    ...
    auto_verify_domains = current_app.config.get("AUTO_VERIFY_ALL_COMPANIES", [])
    ...
    if verify_new_companies:
        ...
        for new_company in new_companies:
            if new_company["domain"] in verify_new_companies:
                verify_none(new_company["company_id"], new_company["domain"], "SYSTEM")
    return True
