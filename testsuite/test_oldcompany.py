...
def test_oldcompany_list_valid(client):
    ...
    factories.OldCompanyFactory(valid=False)
...
def test_oldcompany_list_invalid(client):
    factories.OldCompanyFactory(id=1, valid=True)
...
def test_oldcompany_set_valid(client):
    ...
    resp = client.post(url_for("api.oldcompany-set-valid", pk=oldcompany.external_id))
    data = resp.body
...
def test_oldcompany_set_invalid(client):
    ...
    resp = client.post(url_for("api.oldcompany-set-invalid", pk=oldcompany.external_id))
    data = resp.body
    assert data.decode() == "true"
