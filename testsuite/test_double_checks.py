...
def test_double_checks_fgases_businessprofiles(client):
    with open('testsuite/fixtures/bad_fgas_data/company_fgas_bad_businessprofile.json') as file:
        data = json.load(file)
    result = eea_double_check_fgases(data)
    assert result == False
...
def test_double_checks_ods_country_type(client):
    with open('testsuite/fixtures/bad_ods_data/company_ods_bad_country_type.json') as file:
        data = json.load(file)
    result = eea_double_check_ods(data)
    assert result == False

def test_double_checks_ods_contact_persons(client):
    with open('testsuite/fixtures/bad_ods_data/company_ods_missing_contact_persons.json') as file:
        data = json.load(file)
    result = eea_double_check_ods(data)
    assert result == False
