...
from cache_registry.models import OldCompany, OldCompanyLink, Undertaking
...
def test_auto_verify_companies(client, monkeypatch):
    ...
    undertaking = factories.UndertakingFactory(
        oldcompany=old_company, country_code="FR", vat="1234", oldcompany_verified=False
    )
...
def test_find_company_link(client, monkeypatch):
    monkeypatch.setattr(requests, "get", mockreturn)
    undertaking = factories.UndertakingFactory(
        country_code="FR", vat="1234", oldcompany_verified=False
    )
    old_company = factories.OldCompanyFactory(
        country_code=undertaking.country_code, eori=undertaking.vat
    )
    runner = current_app.test_cli_runner()
    runner.invoke(run, [])
    undertaking = Undertaking.query.first()
