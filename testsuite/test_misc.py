...
def test_export_companies_empty(client):
    resp = client.get(url_for("misc.export-company-list", domain="FGAS"))
...
def test_export_companies(client):
    ...
    resp = client.get(url_for("misc.export-company-list", domain=undertaking.domain))
    assert resp.status_code == 200
    assert resp.content_type == MIMETYPE
    fn = "test_file.xlsx"
    with open(fn, "wb") as f:
...
def test_user_list_export(client):
    ...
    resp = client.get(url_for("misc.export-user-list", domain=FGAS))
...
def test_user_list_export_json(client):
    ...
    undertaking.contact_persons.append(user)
    resp = client.get(url_for("misc.export-user-list-json", domain=FGAS))
    assert resp.status_code == 200
    data = resp.json
...
def test_mail_address_add_new(client):
    resp = client.post(
        url_for("misc.mails-add"), dict(mail="a@ya.com", first_name="a", last_name="b")
    )
    assert resp.status_code == 200
...
def test_mail_address_add_existing(client):
    ma = factories.MailAddress()
    resp = client.post(
        url_for("misc.mails-add"), dict(mail=ma.mail, first_name="a", last_name="b")
    )
    assert resp.status_code == 200
    resp = json.loads(resp.body)
...
def test_mail_address_delete_nonexisting(client):
    ...
    resp = client.post(url_for("misc.mails-delete"), dict(mail="non@existing.com"))
    assert resp.status_code == 200
    resp = json.loads(resp.body)
    assert resp["success"] is False
    assert len(MailAddress.query.all()) == 1
...
def test_mail_alert_wrong_match(client):
    ...
    resp = client.post(
        url_for("misc.alert-wrong-match", domain=undertaking.domain),
...
def test_mail_alert_wrong_lockdown(client):
    ...
    factories.MailAddress()
    resp = client.post(
        url_for("misc.alert-wrong-lockdown", domain=undertaking.domain),
        dict(company_id=undertaking.external_id, user=user.id),
    )
    assert resp.status_code == 200
    assert json.loads(resp.body)
...
def test_crash_me(client):
    client.get(url_for("misc.crashme"), expect_errors=True)
